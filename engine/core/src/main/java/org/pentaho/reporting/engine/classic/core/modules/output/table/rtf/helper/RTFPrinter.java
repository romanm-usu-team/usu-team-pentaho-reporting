...
import org.pentaho.reporting.engine.classic.core.modules.output.table.rtf.itext.PatchRtfBorderGroup;
...
public class RTFPrinter {
    ...
    */
    ...
    private void updateCellStyle(final PatchRtfCell cell, final CellBackground background) {
        ...
        PatchRtfBorderGroup bordersGroup = new PatchRtfBorderGroup();

        final BorderEdge top = background.getTop();
        if (!BorderEdge.EMPTY.equals(top)) {
            bordersGroup.addBorder(Rectangle.TOP,
                    translateBorderStyle(top.getBorderStyle()),
                    (float) StrictGeomUtility.toExternalValue(top.getWidth()),
                    top.getColor());
        ...
        final BorderEdge left = background.getLeft();
        if (!BorderEdge.EMPTY.equals(left)) {
            bordersGroup.addBorder(Rectangle.LEFT,
                    translateBorderStyle(left.getBorderStyle()),
                    (float) StrictGeomUtility.toExternalValue(left.getWidth()),
                    left.getColor());
        }

        final BorderEdge bottom = background.getBottom();
        if (!BorderEdge.EMPTY.equals(bottom)) {
            bordersGroup.addBorder(Rectangle.BOTTOM,
                    translateBorderStyle(bottom.getBorderStyle()),
                    (float) StrictGeomUtility.toExternalValue(bottom.getWidth()),
                    bottom.getColor());
        }

        final BorderEdge right = background.getRight();
        if (!BorderEdge.EMPTY.equals(right)) {
            bordersGroup.addBorder(Rectangle.RIGHT,
                    translateBorderStyle(right.getBorderStyle()),
                    (float) StrictGeomUtility.toExternalValue(right.getWidth()),
                    right.getColor());
        }

        cell.setBorders(bordersGroup);
    }
